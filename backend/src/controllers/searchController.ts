...
export class FlightService {
  async searchFlights(filters: FlightFilters): Promise<FlightResponse[]> {
    try {
      const { date, direction, minPrice, maxPrice, departureTime, arrivalTime } = filters;
      ...
      // Add specific time filters if provided
      if (departureTime) {
        where.departureTime = where.departureTime || {};
        Object.assign(where.departureTime, {
  ...
  async listFlights(filters: FlightFilters): Promise<FlightResponse[]> {
    return this.searchFlights(filters); // Reuse searchFlights functionality
  }
